...
class CIFAR10:
    def __init__(self, path):
        ...
        self.train["labels"] = np.load(os.path.join(path, "labels_.npy")).astype(
            "uint8"
        )
        print("Pre-loading test data")
        self.test["images"] = np.load(os.path.join(path, "test_images.npy")).astype(
            "uint8"
        )
        self.test["hog"] = np.load(os.path.join(path, "test_images_hog.npy"))
        self.test["labels"] = np.load(os.path.join(path, "test_labels.npy")).astype(
            "uint8"
        )
...
def evaluate_classifier(clf, test_data, test_labels):
    ...
    return C.diagonal().sum() * 100.0 / C.sum(), C
    ...
    for r in range(cells):
        for c in range(cells):
            for o, dr, dc in zip(orientations_arr, dr_arr, dc_arr):
                centre = tuple([r * pxcell + pxcell // 2, c * pxcell + pxcell // 2])
                rr, cc = draw.line(
                    int(centre[0] - dc),
                    int(centre[1] + dr),
                    int(centre[0] + dc),
